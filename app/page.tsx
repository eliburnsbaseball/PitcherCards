...
function HaloDot(props: any) {
  ...
  const rOuter = 10 + usage * 0.45;
  const rMid = 6 + usage * 0.25;
  const rCore = 4.5;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs shadow-md dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="text-slate-600 dark:text-slate-300">
        Velo: <span className="font-semibold">{fmt(d.velo, 1)} mph</span> | Usage:{" "}
...
export default function Page() {
  const [dataset, setDataset] = useState<DatasetKey>("2025");
  ...
  // Dark mode toggle (robust: toggles <html class="dark">)
  const [dark, setDark] = useState(false);

  useEffect(() => {
    const saved = typeof window !== "undefined" ? localStorage.getItem("pc_dark") : null;
  ...
  useEffect(() => {
    if (typeof window !== "undefined") localStorage.setItem("pc_dark", dark ? "1" : "0");
    if (typeof document !== "undefined") {
      document.documentElement.classList.toggle("dark", dark);
    }
  }, [dark]);

  // Load index for dropdown when dataset changes
  useEffect(() => {
    setLoading(true);
    setPitcher(null);
    setPlayerMeta(null);
  ...
  // Use mlbam_id for headshot/meta if present
  const metaId = useMemo(() => {
    const mid = pitcher?.mlbam_id ?? selectedId;
  ...
  useEffect(() => {
    ...
    (async () => {
      const res = await fetch(`/api/mlb/player/${metaId}`, { cache: "no-store" });
  ...
  const totalPitches = useMemo(
    () => pitches.reduce((s, p) => s + (Number.isFinite(n(p.count)) ? n(p.count) : 0), 0),
    [pitches]
  );
  ...
  const avgArmAngle = useMemo(() => {
    const vals = pitches.map((p) => p.arm_angle).filter((x) => x != null) as number[];
    if (!vals.length) return NaN;
    return vals.reduce((a, b) => a + b, 0) / vals.length;
  }, [pitches]);

  const headshotUrl = metaId
...
const teamLogoUrl = useMemo(() => {
  ...
  // Arm line: reflect across y-axis if pitcher throws L
  ...
  const armLabel = useMemo(() => {
    if (!Number.isFinite(avgArmAngle)) return "—";
    const base = `${avgArmAngle.toFixed(0)}°`;
    if (dataset === "2026st" && pitcher?.arm_angle_source === "2025") return `${base} (2025)`;
    return base;
  }, [avgArmAngle, dataset, pitcher?.arm_angle_source]);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  <StatPill>{playerMeta?.age != null ? playerMeta.age : "—"}</StatPill>
                  <StatPill>{playerMeta?.height ?? "—"}</StatPill>
                  <StatPill>
                    {playerMeta?.weight != null ? `${playerMeta.weight} lbs` : "—"}
                  </StatPill>
  ...
  <div className="hidden sm:flex h-16 w-16 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/10">
    ...
    />
        ...
        <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
                        ...
                        content={({ active, payload }) => {
                          ...
                          return (
                            <div className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs shadow-md dark:border-slate-700 dark:bg-slate-900">
                              ...
                              <div className="mt-1 text-slate-600 dark:text-slate-300">
                                x: <span className="font-semibold">{fmt(d.x, 2)}</span> | z:{" "}
                                <span className="font-semibold">{fmt(d.z, 2)}</span>
                              </div>
                              <div className="text-slate-600 dark:text-slate-300">
                                Velo: <span className="font-semibold">{fmt(d.velo, 1)} mph</span> | Usage:{" "}
                                <span className="font-semibold">{pct(d.usagePct, 1)}</span>
            ...
            subtitle={
              Number.isFinite(avgArmAngle)
                ? `Arm Angle: ${avgArmAngle.toFixed(0)}°${
                    dataset === "2026st" && pitcher?.arm_angle_source === "2025" ? " (2025)" : ""
                  }`
                : undefined
            }
          >
              ...
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 10, right: 10, bottom: 20, left: 20 }}>
                  ...
                  <XAxis
                    ...
                    domain={[-30, 30]}
                    tick={{ fontSize: 11 }}
                    label={{ value: "HB", position: "insideBottom", offset: -8, fontSize: 11 }}
                  />
                  <YAxis
                    type="number"
                    dataKey="ivb"
                    domain={[-30, 30]}
                    tick={{ fontSize: 11 }}
                    label={{ value: "IVB", angle: -90, position: "insideLeft", fontSize: 11 }}
                  />
          ...
          {/* PITCH FREQUENCY */}
          <Card title="PITCH FREQUENCY" subtitle="Back-to-back: vs LHH (left) / vs RHH (right)">
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
                  data={freqData}
                ...
                >
                  ...
                  />
                  <YAxis type="category" dataKey="code" width={44} tick={{ fontSize: 11, fontWeight: 900 }} />

                  <ReferenceLine x={0} stroke="#e2e8f0" strokeOpacity={0.55} strokeDasharray="2 6" strokeWidth={2} />
                  ...
                  <Bar dataKey="left" barSize={18} isAnimationActive={false}>
                    {freqData.map((d, i) => (
                      <Cell key={i} fill={d.color} opacity={0.85} />
                    ))}
                  </Bar>

                  <Bar dataKey="right" barSize={18} isAnimationActive={false}>
                    {freqData.map((d, i) => (
        ...
        <div className="mt-5 overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[1120px] w-full">
              ...
              <tbody>
                {pitches.map((p) => (
                  <tr key={p.code} className="border-t border-slate-100 text-sm dark:border-slate-800">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className="inline-block h-3 w-3 rounded-sm" style={{ backgroundColor: p.color }} />
                        <div>
                          <div className="font-extrabold text-slate-800 dark:text-slate-100">{p.title}</div>
                    ...
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">{fmt(p.start_speed, 1)} mph</td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">
                      {p.spin_rate == null ? "—" : `${fmt(p.spin_rate, 0)} rpm`}
                    </td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">
                      {p.active_spin_percent == null ? "—" : pct(p.active_spin_percent, 1)}
                    </td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">{fmt(p.ivb, 1)}</td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">{fmt(p.hb, 1)}</td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">{fmt(p.release_pos_z, 2)}</td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">{fmt(p.release_pos_x, 2)}</td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">{fmt(p.extension, 2)}</td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">
                      {p.whiffs_per_pitch == null ? "—" : fmt(p.whiffs_per_pitch, 3)}
                    </td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">
                      {p.swing_miss_percent == null ? "—" : pct(p.swing_miss_percent, 1)}
                    </td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">
                      {p.arm_angle == null ? "—" : `${fmt(p.arm_angle, 0)}°`}
                      {dataset === "2026st" && pitcher?.arm_angle_source === "2025" ? (
                        <span className="ml-1 text-[11px] text-slate-400">(2025)</span>
                      ) : null}
                    </td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">
                      {p.barrels_per_pa_percent == null ? "—" : pct(p.barrels_per_pa_percent, 2)}
                    </td>
                    <td className="px-3 py-3 text-slate-700 dark:text-slate-200">
                      {p.hardhit_percent == null ? "—" : pct(p.hardhit_percent, 1)}
                    </td>
                  </tr>
                ))}
          ...
          <div className="border-t border-slate-200 bg-slate-50 px-4 py-3 text-[11px] font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-400">
            Axes fixed at ±30. ASpin% = Active Spin%.{" "}
            {dataset === "2026st" && pitcher?.arm_angle_source === "2025"
              ? "Arm angle shown is borrowed from 2025."
              : ""}
          </div>
        </div>
      </div>
