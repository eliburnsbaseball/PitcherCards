// scripts/fetchSavant2025aaa.ts
import fs from "fs";
import path from "path";

const OUT_DIR = path.join(process.cwd(), "data", "raw", "2025aaa");
...
function buildUrl(pitchCode: string) {
  ...
  const query =
    ...
    `&hfLevel=${encodeURIComponent("AAA|")}` +
...
async function main() {
  console.log(`📥 Fetching 2025 AAA Savant CSVs → ${OUT_DIR}`);
  for (const code of PITCH_CODES) {
    const url = buildUrl(code);
    const out = path.join(OUT_DIR, `savant_2025aaa_${code}.csv`);
    console.log(`- ${code} …`);
